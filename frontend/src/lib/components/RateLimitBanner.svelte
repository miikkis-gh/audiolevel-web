...
{:else if isLimitReached && status}
  <div class="bg-orange-50 border border-orange-200 rounded-lg p-4">
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1">
        <h3 class="font-medium text-orange-800">Upload limit reached</h3>
        ...
        <div class="mt-3 flex items-center gap-4">
          ...
          <span class="text-xs text-orange-600 font-medium">{status.used}/{status.limit}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
    <div class="flex items-center gap-2 text-sm text-yellow-700">
      ...
      <div class="ml-auto flex items-center gap-2">
        <div class="w-20 bg-yellow-200 rounded-full h-1.5">
          <div
            class="bg-yellow-500 h-1.5 rounded-full"
            style="width: {(status.used / status.limit) * 100}%"
          ></div>
        </div>
        <span class="text-xs text-yellow-600">{status.used}/{status.limit}</span>
      </div>
    </div>
  </div>
{:else if status}
